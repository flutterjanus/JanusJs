...
const updateSubscription = async (subscribeStreams = undefined, unSubscribeStreams = undefined) => {
  if (subscriber) {
    await subscriber.updateAsSubscriber({ subscribe: subscribeStreams, unsubscribe: unSubscribeStreams });
    return;
  }
  ...
  subscriber.onMessage.subscribe(async ({ jsep, message }) => {
    if (jsep) {
      const answer = await subscriber.createAnswer({ jsep, tracks: [{ type: "audio", capture: false, recv: true }] });
  ...
  subscriber.onRemoteTrack.subscribe(({ track, on, mid }) => {
    ...
    if (on) {
      const existingPublisherStream = publisherIdToMediaStream[publisherId];
      if (existingPublisherStream) {
        ...
        if (track.kind === "audio") {
          publisherIdToMediaStream[publisherId].audio = new MediaStream([track]);
        } else if (track.kind === "video") {
          publisherIdToMediaStream[publisherId].video = new MediaStream([track]);
        }
      }
    } else {
      if (track.kind === "audio") {
