...
import { JanusAudioBridgePlugin, JanusJs, JanusStreamingPlugin } from "typed_janus_js";
...
handle1.onMessage.subscribe(async ({ jsep, message }) => {
  ...
  if (message.audiobridge === "joined") {
    const offer = await handle1.createOffer({ tracks: [{ capture: true, recv: false, type: "audio" }] });
    console.log(await handle1.send({ message: { request: "configure" }, jsep: offer }));
  }
  if (jsep) {
...
handle2.onMessage.subscribe(async ({ message, jsep }) => {
  if (jsep) {
    ...
    await handle2.send({ message: { request: "start" }, jsep: answer.toJSON() });
  }
});
...
console.log(list);
console.log(await handle2.send({ message: { request: "watch", id: 5482635511404637 } }));
